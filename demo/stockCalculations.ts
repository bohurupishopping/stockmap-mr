...
import { StockItem, StockSummary, Product, Batch, Transaction, StockFilters as BaseStockFilters } from '@/types/stock';

// Extend StockFilters with pagination properties
interface StockFilters extends BaseStockFilters {
  page?: number;
  pageSize?: number;
}
...
export async function calculateStockData(filters: StockFilters): Promise<StockItem[]> {
  try {
    ...
    // Order by transaction_date in descending order (newest first)
    query = query
...
function processTransaction(
  ...
  if (txType === 'STOCK_IN_GODOWN') {
    // Stock coming into godown (positive quantity_strips in DB)
    ...
    if (transaction.location_type_source === 'GODOWN') {
      ...
      if (locationFilter === 'ALL' || locationFilter === 'GODOWN') {
        ...
        // DB stores outflows as negative values, so we can add directly
